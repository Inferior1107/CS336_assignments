...
\begin{problembox}{cross\_entropy}{Implement Cross entropy}
...
\begin{problembox}{learning\_rate\_tuning}{Tuning the learning rate \hfill (1 point)}
...
\begin{problembox}{adamwAccounting}{Resource accounting for training with AdamW \hfill (2 points)}
...
\begin{answerbox}
    ...
    \[
        \begin{aligned}
            \textbf{Peak Memory}
            = \mathbf{P \times \Big[} & \mathbf{4(2VD + 12LD^2 + 2LD + D)} \\
                                      & \mathbf{+ L(16BSD + BHS^2) + 2BSD + BSV \Big]}
        \end{aligned}
    \]
...
\begin{problembox}{learning\_rate\_schedule}{Implement cosine learning rate schedule with warmup}
...
\begin{problembox}{gradient\_clipping}{Implement gradient clipping \hfill (1 point)}
